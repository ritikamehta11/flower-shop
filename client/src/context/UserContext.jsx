...
export const UserProvider = ({ children }) => {
      ...
      const fetchCart = async () => {
        try {
          const response = await axios.get(
            `https://flower-shop-1qtms7z64-ritika-mehtas-projects.vercel.app/api/cart/${user.id}`,
            {
              headers: {
  ...
  const addToCart = async (pid, quantity) => {
    ...
    try {
      const response = await axios.post(
        "https://flower-shop-1qtms7z64-ritika-mehtas-projects.vercel.app/api/cart",
  ...
  const removeFromCart = async (id) => {
    ...
    try {
      await axios.delete(
        `https://flower-shop-1qtms7z64-ritika-mehtas-projects.vercel.app/api/cart/${user._id}/product/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const increaseQuantity = async (pid) => {
    ...
    try {
      await axios.patch(
        `https://flower-shop-1qtms7z64-ritika-mehtas-projects.vercel.app/api/cart/${user._id}/product/${pid}/increase`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const decreaseQuantity = async (pid) => {
    ...
    try {
      await axios.patch(
        `https://flower-shop-1qtms7z64-ritika-mehtas-projects.vercel.app/api/cart/${user._id}/product/${pid}/decrease`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
